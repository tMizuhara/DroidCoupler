...
public class BinarySoundEncoder {
	...
	public void encByteSound(byte data, byte[] sound) {
		...
		for (int i = 0; i < dL; ++i, ++t) {
			sound[t] = (byte) (Byte.MAX_VALUE * Math.sin(2.0 * Math.PI * t * fs
					/ L));
		}
		for (int bit = 0; bit < 8; ++bit) {
			...
			if (((data >> bit) & 0x01) == 0x01) {
				for (int i = 0; i < dL; ++i, ++t) {
					sound[t] = (byte) (Byte.MAX_VALUE * Math.sin(2.0 * Math.PI
							* t * f1 / L));
				}
			} else {
				for (int i = 0; i < dL; ++i, ++t) {
		...
		// stop bit
		for (int i = 0; i < dL; ++i, ++t) {
			sound[t] = (byte) (Byte.MAX_VALUE * Math.sin(2.0 * Math.PI * t * fe
					/ L));
		}
	}
